...
def summarize_transactions(df: pd.DataFrame) -> dict:
    ...
    Args:
        df: Cleaned DataFrame with 'amount' column.
...
def category_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df: Cleaned DataFrame with 'amount' and 'category' columns.
    ...
    summary = summary.sort_values(by="total_spent", ascending=False).reset_index(drop=True)
